#!/usr/bin/env node
// Imports markdown docs from a source folder (default: SD-Backend/docs) into the running SdDocs API as r1
// revisions, using the same heuristics as the Phase 0 mock seed (web/scripts is now scripts/seed-mock.mjs).
// Not run automatically — invoke deliberately: node scripts/import.mjs [--if-empty]
//   source override: SD_DOCS_SOURCE=<path to docs folder>
//   API override:    SD_DOCS_API_URL=<url> (default http://localhost:5180)
import { existsSync, readdirSync, readFileSync, statSync } from 'node:fs'
import { join, relative, resolve } from 'node:path'

const source = resolve(process.env.SD_DOCS_SOURCE || join(import.meta.dirname, '..', '..', 'SD', 'SD-Backend', 'docs'))
const apiBase = (process.env.SD_DOCS_API_URL || 'http://localhost:5180').replace(/\/+$/, '')
...
async function apiSave(id, header, content) {
  const res = await fetch(`${apiBase}/api/doc?author=Vladyslav&source=import`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id, header, content, message: 'Imported from SD-Backend/docs', ticket: null, baseRevision: null }),
  })
...
async function main() {
  if (!existsSync(source)) {
    console.error(`import: source docs folder not found: ${source} (set SD_DOCS_SOURCE)`)
    process.exit(1)
  }
